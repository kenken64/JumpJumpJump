...
export class WorldGenerator {
  ...
  generateChunk(startX: number) {
    ...
    const yLevels = [300, 400, 500, 600, 700, 800] // 6 levels with 100px spacing
      ...
      if (structureType < 0.3) {
        ...
        if (!occupiedCells.has(cellKey)) {
          for (let i = 0; i < platformWidth; i++) {
            ...
            // Floating platform hitbox - minimal to allow player through 70px gaps
        ...
        if (canPlace) {
          for (let i = 0; i < steps; i++) {
            ...
            // Staircase platform hitbox - minimal to allow player through 70px gaps
