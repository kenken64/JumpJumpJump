      ...
      if (structureType < 0.3) {
        ...
        if (!occupiedCells.has(cellKey)) {
          for (let i = 0; i < platformWidth; i++) {
            ...
            // Floating platform hitbox - extremely thin to allow player through gaps
            const hitboxWidth = plat.width * 0.4  // 40% width
            const hitboxHeight = plat.height * 0.1  // 10% height - ultra thin
            body.setSize(hitboxWidth, hitboxHeight)
        ...
        if (canPlace) {
          for (let i = 0; i < steps; i++) {
            ...
            // Staircase platform hitbox - extremely thin to allow player through gaps
            const hitboxWidth = step.width * 0.4  // 40% width
            const hitboxHeight = step.height * 0.1  // 10% height - ultra thin
            body.setSize(hitboxWidth, hitboxHeight)
            // Align hitbox to TOP of platform sprite
